...
export const ReportService = {
  ...
  async downloadExcel(data: TemuanData[], filters: any) {
    ...
    worksheet.getCell('A3').value = 'TIM DIVISI INSPEKSI PLN ELECTRICITY SERVICES UL PADANG';
    ...
    const rowSig5 = worksheet.addRow(['', '', '', '', '', '', '', '', '', 'ADM INSPEKSI', `: `]);
  ...
  async downloadPDF(data: TemuanData[], filters: any) {
    ...
    doc.setFontSize(10);
    doc.text('TIM DIVISI INSPEKSI PLN ELECTRICITY SERVICES UL PADANG', 148, 28, { align: 'center' });
  ...
  async downloadYandalExcel(yandalData: { name: string, ulp: string, total: number, period: string }[], filters: { start: string, end: string, ulp: string }) {
    ...
    worksheet.getCell('A3').value = 'PLN ELECTRICITY SERVICES UL PADANG';
    ...
    const sigRow1 = worksheet.addRow(['', '', '', 'Padang, ' + dateStr]);
    const sigRow2 = worksheet.addRow(['', '', '', 'ADM INSPEKSI']);
    worksheet.addRow([]);
    worksheet.addRow([]);
    worksheet.addRow([]);
    const sigRow6 = worksheet.addRow(['', '', '', '']);
